...
class CalculatorServiceTests(TestCase):
    def test_calculate_dosage_mg(self):
        ...
        self.assertEqual(dosage, Decimal('750.00'))  # 50 mg/kg * 15 kg = 750 mg

        pacient_weight = 20  # kg
        medication_prescription = 25 #mg/kg
        dosage = services.calculate_dosage_mg(medication_prescription, pacient_weight)
        self.assertEqual(dosage, Decimal('500.00'))  # 25 mg/kg * 20 kg = 500 mg
        ...
        self.assertEqual(dosage, Decimal('57.74')) # Superfície corporal = sqrt((15*80)/3600) = 0.577 m2, Dosagem = 100 mg/m2 * 0.577 m2 = 57.7 mg
    ...
    def test_calculate_dosage_per_dose(self):
        self.assertEqual(services.calculate_dosage_per_dose(750, 3), Decimal('250.00'))  # 750 mg por dia dividido em 3 doses = 250 mg por dose
        self.assertEqual(services.calculate_dosage_per_dose(1200, 4), Decimal('300.00')) # 1200 mg por dia dividido em 4 doses = 300 mg por dose

    def test_dosage_to_ml_conversion(self):
        self.assertEqual(services.convert_dosage_to_ml(250, 50, 5), Decimal('25.00'))  # 250 mg dividido pela concentração de 50 mg/5 ml = 25 ml
        self.assertEqual(services.convert_dosage_to_ml(300, 100, 10), Decimal('30.00')) # 300 mg dividido pela concentração de 100 mg/10 ml = 30 ml
